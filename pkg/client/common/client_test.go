...
// GetName is an alias for GetRepositoryName
...
func (m *mockRepository) GetLayerReader(digest string) (io.ReadCloser, error) {
	if m.layerError != nil {
		return nil, m.layerError
	}

	// For testing, just return a simple reader with the digest as content
	return io.NopCloser(strings.NewReader("mock layer content for " + digest)), nil
}
