...
// validateReplicateRequest validates a replicate request
func (s *Server) validateReplicateRequest(req *ReplicateRequest) error {
	...
	if !isValidRegistryType(req.SourceRegistry) {
		return fmt.Errorf("invalid source registry type: %s", req.SourceRegistry)
	}

	if !isValidRegistryType(req.DestRegistry) {
		return fmt.Errorf("invalid destination registry type: %s", req.DestRegistry)
	}

	return nil
}

// validateReplicateTreeRequest validates a replicate tree request
...
// isValidRegistryType checks if a registry type is valid
