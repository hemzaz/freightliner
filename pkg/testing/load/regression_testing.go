...
func (rts *RegressionTestSuite) StartAutomatedTesting(ctx context.Context) error {
	rts.logger.Info("Starting automated regression testing", map[string]interface{}{
	...
	if err := rts.baselineManager.LoadBaselines(); err != nil {
		rts.logger.Warn("Failed to load baselines", map[string]interface{}{"error": err.Error()})
	}
