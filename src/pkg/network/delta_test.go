...
func TestCompareChunks(t *testing.T) {
	...
	changed, total = compareChunks(source, dest)
	// The last chunk of the original data plus a new chunk are different
	if len(changed) != 2 {
		t.Errorf("Expected 2 changed chunk for additional data, got %d", len(changed))
	}
}
...
func TestOptimizeTransfer(t *testing.T) {
	...
	// The changes might be significant enough to cause a full transfer
	// so we should not fail if it's not optimized
	if wasOptimized {
		// Only check savings if we actually did a delta transfer
		if summary != nil && summary.SavingsPercent <= 0 {
